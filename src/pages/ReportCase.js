...
const SEVERITIES = [
  { v: 'low',      label: 'Low',      dot: '#10b981' },
  { v: 'medium',   label: 'Medium',   dot: '#f59e0b' },
  { v: 'high',     label: 'High',     dot: '#ef4444' },
  { v: 'critical', label: 'Critical', dot: '#dc2626' },
];
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const result = await submitReport(
        { ...data, severity, isAnonymous: isAnon },
        files,
        (idx, total, pct) => setProgress(`Uploading file ${idx + 1}/${total} — ${pct}%`)
      );
      setDone(result);
      reset();
      setFiles([]);
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 'calc(100vh - 68px)', padding: 24, background: '#f9fafb' }} className="fade-in">
        <div style={{ width: '100%', maxWidth: 460, background: 'white', borderRadius: 20, border: '1.5px solid #e5e7eb', overflow: 'hidden', boxShadow: '0 8px 32px rgba(0,0,0,0.08)' }}>
          <div style={{ background: 'linear-gradient(135deg,#0ea5e9,#818cf8,#ec4899)', padding: '36px', textAlign: 'center' }}>
            <div style={{ width: 64, height: 64, borderRadius: 16, background: 'rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px', color: 'white' }}>
              <IcoCheck />
            </div>
          ...
          <div style={{ padding: '28px 32px 32px' }}>
            <p style={{ fontSize: '0.85rem', color: '#6b7280', lineHeight: 1.7, marginBottom: 14, textAlign: 'center' }}>
              Save your tracking ID to follow your case:
            </p>
            <div style={{ background: '#f0f9ff', border: '1.5px solid #bae6fd', borderRadius: 10, padding: '16px', fontFamily: 'monospace', fontSize: '1.1rem', color: '#0ea5e9', textAlign: 'center', letterSpacing: '0.1em', fontWeight: 700, marginBottom: 8, wordBreak: 'break-all' }}>
              {done.trackId}
            </div>
            <p style={{ fontSize: '0.75rem', color: '#9ca3af', textAlign: 'center', marginBottom: 24 }}>No personal data was stored.</p>
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
              <button className="btn btn-ghost" style={{ fontSize: '0.85rem' }} onClick={() => setDone(null)}>
                Submit Another
              </button>
    ...
    <div className="report-wrap fade-in" style={{ background: '#f9fafb', minHeight: 'calc(100vh - 68px)', padding: '48px 24px' }}>
      <div style={{ maxWidth: 720, margin: '0 auto' }}>
        ...
        <p style={{ color: '#6b7280', marginBottom: 28, lineHeight: 1.6 }}>
          Your identity is protected. All information is encrypted and handled confidentially.
        </p>
        ...
        <div className="report-card" style={{ background: 'white', border: '1.5px solid #e5e7eb', borderRadius: 20, padding: '28px', boxShadow: '0 4px 16px rgba(0,0,0,0.06)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#f0f9ff', border: '1.5px solid #bae6fd', borderRadius: 12, padding: '14px 18px', marginBottom: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                <strong style={{ fontSize: '0.88rem', color: '#0369a1', display: 'block' }}>
                  Anonymous Mode {isAnon ? 'Enabled' : 'Disabled'}
                </strong>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="field">
              ...
              <textarea
                ...
                {...register('description', { required: 'Required', minLength: { value: 20, message: 'At least 20 characters' } })}
                placeholder="Describe what happened — dates, locations, people involved…"
            ...
            <div className="field">
              ...
              >
                ...
                <p style={{ fontSize: '0.82rem', color: '#9ca3af' }}>
                  <span style={{ color: '#0ea5e9', fontWeight: 600 }}>Click to upload</span> or drag &amp; drop
                </p>
                ...
                <div style={{ marginTop: 8, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {files.map((f, i) => (
                    <span key={i} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: '#f0f9ff', border: '1px solid #bae6fd', borderRadius: 20, padding: '3px 10px', fontSize: '0.75rem', color: '#0369a1' }}>
                      {f.name}
                      <span onClick={() => setFiles(files.filter((_, j) => j !== i))} style={{ cursor: 'pointer', color: '#ef4444', display: 'flex' }}>
                        <IcoX />
                      </span>
                    </span>
                  ))}
            ...
            <div className="field">
              <label className="field-label">
                Contact Email <span style={{ fontWeight: 400, color: '#9ca3af' }}>(optional — for updates only)</span>
              </label>
