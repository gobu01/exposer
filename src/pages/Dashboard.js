...
export default function Dashboard() {
  ...
  const handleStatusChange = async (reportId, newStatus) => {
    ...
    try {
      await updateReportStatus(reportId, newStatus, profile?.uid);
  ...
  const statusData = Object.entries(
    reports.reduce((a, r) => ({ ...a, [r.status]: (a[r.status] || 0) + 1 }), {})
  ).map(([name, value]) => ({ name, value }));

  const sevData = ['critical', 'high', 'medium', 'low']
    .map(s => ({ name: s, value: reports.filter(r => r.severity === s).length }))
    .filter(d => d.value > 0);
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: '#9ca3af', fontSize: '0.85rem' }}>Loading reports…</p>
        </div>
      </div>
    );
  }
              ...
              <ResponsiveContainer width="100%" height={150}>
                <BarChart data={statusData} margin={{ top: 0, right: 0, bottom: 0, left: -20 }}>
                  ...
                  <Bar dataKey="value" radius={[4, 4, 0, 0]}>
                    {statusData.map((e, i) => <Cell key={i} fill={STATUS_COLORS[e.name] || '#60a5fa'} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
          ...
          {filtered.length === 0 ? (
            ...
            <div className="dash-table-wrap" style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    {['Tracking ID', 'Title', 'Category', 'Severity', 'Status', 'Filed', 'Actions'].map(h => (
                      <th key={h} style={{ padding: '11px 20px', textAlign: 'left', fontSize: '0.72rem', color: '#9ca3af', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', background: '#fafafa', borderBottom: '1px solid #f3f4f6' }}>
                        {h}
                      </th>
                    ))}
                  </tr>
                    ...
                    >
                      ...
                      <td style={{ padding: '13px 20px' }}>
                        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                          <select
                            onChange={e => { if (e.target.value) handleStatusChange(r.id, e.target.value); e.target.value = ''; }}
                          ...
                          >
                            ...
                            {['pending', 'reviewing', 'investigating', 'escalated', 'resolved', 'dismissed'].map(s => (
