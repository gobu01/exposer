...
const fmt=(ts)=>{if(!ts)return'—';try{const d=ts.toDate?ts.toDate():new Date(ts);return d.toLocaleString('en-IN');}catch{return'—';}};
...
export default function ReportDetail() {
  ...
  const load=async()=>{
    ...
    try{const r=await getReport(id);if(!r)setError('Report not found.');else setReport(r);}
    catch(err){setError('Failed to load: '+(err.message||'Unknown error')+'. Check your Firestore rules.');}
  ...
  const handleStatus=async(status)=>{
    ...
    setUpdating(true);
    try{await updateReportStatus(id,status,profile?.uid);toast.success('Status → '+status);setReport(r=>({...r,status}));}
  ...
  const handleComment=async()=>{
    ...
    setPosting(true);
    try{await addComment(id,profile?.uid,profile?.name||'Staff',comment.trim(),isInternal);toast.success('Note added');setComment('');await load();}
  ...
  if(loading)return(
    <div style={{display:'flex',alignItems:'center',justifyContent:'center',minHeight:'60vh'}}>
      <div style={{textAlign:'center'}}><div className="spinner spinner-blue" style={{width:36,height:36,borderWidth:3,margin:'0 auto 14px'}}/><p style={{color:'#9ca3af',fontSize:'0.85rem'}}>Loading report…</p></div>
    </div>
  );

  if(error)return(
    <div style={{maxWidth:540,margin:'80px auto',padding:'0 24px',textAlign:'center'}}>
      <div style={{width:52,height:52,borderRadius:14,background:'#fef2f2',border:'1.5px solid #fecaca',display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 14px',color:'#ef4444'}}><IcoAlert/></div>
      <h2 style={{fontWeight:800,marginBottom:10,fontSize:'1.3rem'}}>Could Not Load Report</h2>
      <div style={{background:'#fef2f2',border:'1.5px solid #fecaca',borderRadius:10,padding:'12px 16px',fontSize:'0.8rem',color:'#b91c1c',marginBottom:18,textAlign:'left',fontFamily:'monospace',lineHeight:1.7}}>{error}</div>
      <div style={{display:'flex',gap:8,justifyContent:'center'}}>
        <button className="btn btn-primary" style={{background:'#0ea5e9',boxShadow:'0 4px 12px rgba(14,165,233,0.3)',fontSize:'0.85rem'}} onClick={load}>Retry</button>
        <button className="btn btn-ghost" style={{fontSize:'0.85rem'}} onClick={()=>navigate('/dashboard')}>Back to Dashboard</button>
      </div>
    </div>
  );
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 290px',gap:18,alignItems:'start'}}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:16}}>
            ...
            {report.evidence?.length>0&&(
              <div style={{background:'white',border:'1.5px solid #e5e7eb',borderRadius:16,padding:'22px',boxShadow:'0 2px 8px rgba(0,0,0,0.04)'}}>
                <h3 style={{fontSize:'0.88rem',fontWeight:700,color:'#111827',marginBottom:12}}>Evidence ({report.evidence.length})</h3>
            ...
            <div style={{background:'white',border:'1.5px solid #e5e7eb',borderRadius:16,padding:'22px',boxShadow:'0 2px 8px rgba(0,0,0,0.04)'}}>
              <h3 style={{fontSize:'0.88rem',fontWeight:700,color:'#111827',marginBottom:14}}>Investigation Notes ({(report.comments||[]).length})</h3>

              {(report.comments||[]).length>0?(
                <div style={{display:'flex',flexDirection:'column',gap:10,marginBottom:18}}>
                  {report.comments.map(c=>(
                    <div key={c.id} style={{padding:'12px 14px',borderRadius:10,background:c.isInternal?'#fefce8':'#f9fafb',border:'1.5px solid '+(c.isInternal?'#fde047':'#e5e7eb')}}>
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:7,flexWrap:'wrap',gap:5}}>
                        ...
                        <div style={{display:'flex',gap:6,alignItems:'center'}}>
                          {c.isInternal&&<span style={{fontSize:'0.65rem',background:'#fef9c3',color:'#854d0e',padding:'2px 7px',borderRadius:10,fontWeight:600}}>Internal</span>}
              ...
              <div style={{borderTop:'1px solid #f3f4f6',paddingTop:14}}>
                <textarea className="field-input" value={comment} onChange={e=>setComment(e.target.value)}
                  placeholder="Add an investigation note…" style={{minHeight:80,marginBottom:10,resize:'vertical'}}/>
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10}}>
                  <label style={{display:'flex',alignItems:'center',gap:7,cursor:'pointer',fontSize:'0.82rem',color:'#374151'}}>
                    <input type="checkbox" checked={isInternal} onChange={e=>setIsInternal(e.target.checked)} style={{width:15,height:15,accentColor:'#f59e0b'}}/>
                    Internal only
                  </label>
                  <button className="btn btn-primary" style={{padding:'8px 16px',fontSize:'0.82rem',gap:5,background:'#0ea5e9',boxShadow:'0 4px 12px rgba(14,165,233,0.25)'}} onClick={handleComment} disabled={posting||!comment.trim()}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:14}}>
            <div style={{background:'white',border:'1.5px solid #e5e7eb',borderRadius:16,padding:'20px',boxShadow:'0 2px 8px rgba(0,0,0,0.04)'}}>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:5}}>
                {STATUS_OPTS.map(s=>(
                  <button key={s} onClick={()=>handleStatus(s)} disabled={report.status===s||updating} style={{
                    ...
                    textAlign:'left',textTransform:'capitalize',transition:'all 0.12s',
              ...
              {updating&&<div style={{display:'flex',alignItems:'center',gap:7,marginTop:10,fontSize:'0.75rem',color:'#9ca3af'}}><div className="spinner spinner-blue" style={{width:13,height:13}}/>Updating…</div>}
            </div>

            <div style={{background:'white',border:'1.5px solid #e5e7eb',borderRadius:16,padding:'20px',boxShadow:'0 2px 8px rgba(0,0,0,0.04)'}}>
              <h3 style={{fontSize:'0.88rem',fontWeight:700,color:'#111827',marginBottom:12}}>Report Info</h3>
              {[['Category',(report.category||'').replace(/_/g,' ')],['Severity',report.severity],['Location',report.location||'—'],['Incident',report.incidentDate||'—'],['Filed',fmt(report.createdAt)],['Updated',fmt(report.updatedAt)],['Anonymous',report.isAnonymous?'Yes':'No'],['Email',report.reporterEmail||'Hidden'],['Assigned',report.assignedTo||'Unassigned']].map(([k,v])=>(
                <div key={k} style={{display:'flex',justifyContent:'space-between',padding:'6px 0',borderBottom:'1px solid #f9fafb',gap:8}}>
                  ...
                  <span style={{fontSize:'0.73rem',color:'#374151',fontWeight:500,textAlign:'right',textTransform:'capitalize',wordBreak:'break-word',maxWidth:'60%'}}>{v}</span>
                </div>
              ))}
            </div>
          </div>
